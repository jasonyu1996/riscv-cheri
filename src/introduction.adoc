...
the RISC-V unprivileged ISA (specifically, RV32I and RV64I). The resulting extended ISA is not
backwards-compatible with RISC-V
{cheri_priv_ext_name}:: Extends {cheri_base_ext_name} with integration with
the privileged architecture of RISC-V, including the privileged control and
status registers (CSRs) and instructions defined in Zicsr
...
{cheri_priv_ext_name}, {cheri_legacy_ext_name}, {cheri_mode_ext_name}
and {cheri_pte_ext_name} are optional extensions in addition to
{cheri_base_ext_name}.
{cheri_priv_ext_name} extends {cheri_base_ext_name} with integration
with the standard RISC-V privileged architecture, and is required by
{cheri_legacy_ext_name}, {cheri_mode_ext_name}, and {cheri_pte_ext_name}.
{cheri_mode_ext_name} further requires supporting both
...
requires the CHERI RISC-V hart to support {cheri_base_ext_name}, {cheri_priv_ext_name},
{cheri_legacy_ext_name} and {cheri_mode_ext_name}.

TODO: See whether we can remove the requirement on {cheri_priv_ext_name} from hybrid.
